...
def load_logo(url, size):
    """Baixa o logo da URL e redimensiona mantendo proporção."""
    if not url:
        return None
    try:
...
def draw_tv_icon(draw, cx, cy, size, color):
    """Desenha ícone de TV vetorial quando não há logo."""
    r = size // 2
...
def generate_android_banner(out_path):
    """Gera banner 320x180 para Android TV."""
    W, H = 320, 180
    ...
    if logo:
        # Centralizar logo no círculo
        lw, lh = logo.size
        lx = cx - lw // 2
        ly = cy - lh // 2
        img.paste(logo, (lx, ly), logo)
    else:
        # Fallback: ícone de TV vetorial
    ...
    # Texto do app name (lado direito)
    area_x = 160
    ...
    # Quebrar nome em palavras para melhor layout
...
def generate_roku_icon(out_path, w, h, logo_size):
    """Gera ícone Roku com o logo do revendedor."""
    ...
    if logo:
        ...
        tv_draw = ImageDraw.Draw(img)
        r = logo_size // 2
        tv_draw.ellipse([cx-r-2, cy-r-2, cx+r+2, cy+r+2], fill=tuple(int(v*0.6) for v in PRIMARY_RGB) + (255,))
        tv_draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=BG_RGB + (255,))
        draw_tv_icon(tv_draw, cx, cy, logo_size - 10, PRIMARY_RGB)
...
def generate_roku_splash(out_path):
    """Gera splash screen Roku 1280x720."""
    W, H = 1280, 720
    img = Image.new("RGB", (W, H), BG_RGB)
    draw = ImageDraw.Draw(img)
...
if __name__ == "__main__":
    ...
    # Android TV banner (todas as densidades usam o mesmo)
