...
def draw_tv_icon(draw, cx, cy, size, color):
    ...
    draw.rounded_rectangle([tx+4, ty+4, tx+tw-4, ty+th-4], radius=max(2, size//20), fill=BG_RGB)
...
def generate_android_banner(out_path):
    ...
    cx, cy = 80, 90
    logo = load_logo(APP_LOGO_URL, 120)
    if logo:
        lw, lh = logo.size
    ...
    lines = [" ".join(words[:-1]), words[-1]] if len(words) >= 2 else [APP_NAME]
    total = 42 * len(lines)
    y = (H - total) // 2
    for line in lines:
        b = font.getbbox(line)
        x = center_x - (b[2] - b[0]) // 2
        draw.text((x, y), line, font=font, fill=PRIMARY_RGB)
        y += 42
