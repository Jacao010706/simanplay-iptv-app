...
"""
import os, sys, shutil, zipfile, urllib.request, io
...
def generate_icon(size):
    """Gera ícone do app com o logo do revendedor."""
...
def replace_placeholders(content):
    """Substitui todos os placeholders com dados do revendedor."""
    return (content
...
def build_samsung_wgt():
    """Empacota o app Samsung Tizen como .wgt (zip renomeado)."""
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(src):
            for fname in files:
                ...
                # Substituir placeholders em arquivos de texto
                if fpath.endswith((".html", ".js", ".css", ".xml", ".json")):
                    content = replace_placeholders(raw.decode("utf-8", errors="ignore"))
                    raw = content.encode("utf-8")

                zf.writestr(arcname, raw)

        # Adicionar ícones gerados com logo do revendedor
...
def build_lg_ipk():
    """Empacota o app LG webOS como .ipk."""
    print("\n📦 Empacotando LG webOS (.ipk)...")
    src = "lg_webos"
    out = "simanplay_lg.ipk"

    # .ipk é um ar archive com: control.tar.gz + data.tar.gz + debian-binary
    # Simplificado: usamos .zip com estrutura webOS
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(src):
            for fname in files:
                fpath = os.path.join(root, fname)
