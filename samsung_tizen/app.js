// ============================================================
// SimanPlay IPTV - Samsung Tizen App
// Controle de navegação e reprodução
// ============================================================
...
function navigate(dir) {
    ...
    const focusable = Array.from(document.querySelectorAll('[data-focus]')).filter(el => el.offsetParent !== null);
...
function loginXtream() {
    ...
    const status = document.getElementById('loginStatus');
    if (!host || !user || !pass) { status.textContent = 'Preencha todos os campos'; return; }
...
async function loadChannels(categoryId, categoryName, elCat) {
...
function playChannel(ch) {
    ...
    const video = document.getElementById('videoPlayer'); video.src = url; video.play().catch(() => {});
}

function stopPlayer() {
    const video = document.getElementById('videoPlayer'); video.pause(); video.src = '';
    currentSection = 'home';
    document.getElementById('playerScreen').style.display = 'none';
    document.getElementById('homeScreen').style.display = 'flex';
}
